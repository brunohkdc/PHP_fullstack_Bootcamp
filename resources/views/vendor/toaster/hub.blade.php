<div role="status" id="toaster" x-data="toasterHub(@js($toasts), @js($config))" @class([
    'fixed z-50 p-4 w-5/6 flex flex-col pointer-events-none sm:p-6',
    ...
    <template x-for="toast in toasts" :key="toast.id">
        <div x-show="toast.isVisible"
             ...
             @class(['relative duration-300 transform transition ease-in-out max-w-xs w-5/6 pointer-events-auto', 'text-center' => $position->is('center')])
             :class="toast.select({ error: 'text-white', info: 'text-white', success: 'text-white', warning: 'text-white' })"
        >
            <i x-text="toast.message"
               class="inline-block select-none not-italic px-6 py-3 rounded rounded-sm shadow-lg text-sm w-5/6 {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }}"
               :class="toast.select({ error: 'bg-red-500', info: 'bg-blue-700', success: 'bg-green-600', warning: 'bg-orange-500' })"
            ></i>
